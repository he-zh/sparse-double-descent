...
class RewindingRunner(Runner):
    ...
    @staticmethod
    def description():
        return 'Run a pruning and rewinding experiment.'
    ...
    def _add_levels_argument(parser):
        help_text = \
            ...
            'completion, pruned, and rewound, preparing it for the next iteration. The full network is trained ' \
    ...
    def add_args(parser: argparse.ArgumentParser) -> None:
        ...
        rewinding_parser = parser.add_argument_group(
        'Rewinding Hyperparameters', 'Hyperparameters that control the pruning and rewinding process.')
        RewindingRunner._add_levels_argument(rewinding_parser)
        RewindingDesc.add_args(parser, defaults)

    @staticmethod
    def create_from_args(args: argparse.Namespace) -> 'RewindingRunner':
        return RewindingRunner(args.replicate, args.levels, RewindingDesc.create_from_args(args),
    ...
    def _train_level(self, level: int):
        ...
        # use the final weight values from the end of training from the previous level
        if level != 0:
            model = models.registry.load(self.desc.run_path(self.replicate, level-1), self.desc.train_end_step,
                                         self.desc.model_hparams, self.desc.train_outputs, 
                                         self.desc.pruning_hparams.pruning_strategy)
        # else use the initial weight values from level 0
        else:
