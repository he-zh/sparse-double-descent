...
class DatasetHparams(Hparams):
    ...
    noisy_labels_fraction: float = None
    noisy_labels_type: str = None
    ...
    _noisy_labels_fraction: str =  'Apply noisy labels to a fraction of the training set: float in (0, 1]'
    _noisy_labels_type: str = 'Specify the type of noisy labels. Examples: symmetric, asymmetric'
...
class TrainingHparams(Hparams):
    ...
    regain_pruned_weights_steps : str = None
    use_convergence_stopping_criterion : bool = False
    bn_sparsity_regularization: bool = False
    distance_penalty: float = None
    _convergence_training_steps : str = None
    ...
    _nesterov_momentum: bool = 'The nesterov momentum to use with the SGD optimizer. Cannot set both momentum and nesterov.'
    ...
    _regain_pruned_weights_steps: str = 'The number of steps to mask pruned weights with 0 as epochs (\'2ep\') or iterations (\'2000it\'). '\
                                        'After training for such steps, pruned weights will be allowed to be trainable and updated from 0. '\
                                        'The default None perform regular pruning scheme.'
    _bn_sparsity_regularization: str = 'If true, additional l1 regularization will be applied to batch normalization layers for network-slimming pruning'
    _distance_penalty: str = 'The L2 penalty to apply to the distance of weights from initialization.'
    _use_convergence_stopping_criterion: str = 'If True, training process will be stopped when either cross-entropy loss is lower '\
                                               'than 0.01, or the number of training iterations runs out of acceptable range. '

@dataclass
class PruningHparams(Hparams):
    pruning_strategy: str
    freeze_pruned_weights: str = 'zero'
    ...
    _freeze_pruned_weights = 'A parameter to control that the pruned weighst to be frozen at zero or the final value before pruning, or decay to 0 gradually, '\
                             ' choose from [zero, init, final]'
    

@dataclass
class FinetuningHparams(Hparams):
    lr: float
    training_steps: str
    milestone_steps: str = None
    gamma: float = None
    ...
    _name: str = 'Finetuning Hyperparameters'
    _description: str = 'Hyperparameters that determine how the model is finetuned.'
    _lr: str = 'The learning rate'
    _training_steps: str = 'The number of steps to train as epochs (\'160ep\') or iterations (\'50000it\').'
    _milestone_steps: str = 'Steps when the learning rate drops by a factor of gamma. Written as comma-separated ' \
                            'steps (80ep,160ep,240ep) where steps are epochs (\'160ep\') or iterations (\'50000it\').'
