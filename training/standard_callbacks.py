...
def create_eval_callback(eval_name: str, loader: DataLoader, verbose=False):
    ...
    def eval_callback(output_location, step, model, optimizer, logger):
        example_count = torch.tensor(0.0).to(Platform().torch_device)
        total_loss = torch.tensor(0.0).to(Platform().torch_device)
        total_correct = torch.tensor(0.0).to(Platform().torch_device)
        ...
        with torch.no_grad():
            for examples, labels in loader:
                examples = examples.to(Platform().torch_device)
                labels = labels.squeeze().to(Platform().torch_device)
                output = model(examples)

                labels_size = torch.tensor(len(labels), device=Platform().torch_device)
